...
from cum_split import molecular_dataset_split
# ------------------------------
...
def get_global_features(mol):
    ...
    return [mw, tpsa, num_rings, logp, num_val, lip_HBA, lip_HBD, rot_bond]
...
def compute_hat_matrix_approx(features,preds, trues):
...
def williams_plot(train, val, test, train_pred, val_pred, test_pred, train_nom, val_nom, test_nom, train_idx, val_idx, test_idx, output_dir):
    ...
    def notation(h, h_crit, std_residuals, idx):
        ...
        for i in range(len(h)):
            ...
            if h[i] > h_crit or abs(std_residuals[i]) > 3:
    
                x = h[i]
                y = std_residuals[i]
    
                # Count nearby already-annotated points
                overlap_count = 0
    
                for px, py in annotated_points:
                    if abs(x - px) < 0.02 and abs(y - py) < 0.2:
                        overlap_count += 1
    
                # Place labels around the point
                offsets = [
                    (10, 10),
                    (-10, 10),
                    (10, -10),
                ...
                dx, dy = offsets[overlap_count % len(offsets)]
...
def plot_true_vs_predicted(
    ...
    ax.plot(x_line, predictions.predicted_mean, color=color, linewidth=2)
...
# Format the plot
    ...
    plt.xlim(min(x_line)-1,max(x_line)+1)

    plt.ylim(min(x_line)-1,max(x_line)+1)

    ax.set_xlabel('True pAffinity', fontsize=12)

    ax.set_ylabel('Predicted pAffinity', fontsize=12)

    ax.set_title(f'True vs. Predicted pAffinity ({dataset_label})', fontsize=14)
    ...
    def residuals_histogram(preds, trues, output_path):
    ...
    hist_path = os.path.join(output_dir, f"Histogram_Residuals_{dataset_label}_set.png")

    residuals_histogram(preds=y, trues=x, output_path=hist_path)
...
def plot_true_vs_predicted_plotly(
    ...
    fig.update_layout(
        ...
        xaxis=dict(range=[x.min() - 1, x.max() + 1]),
        yaxis=dict(range=[y.min() - 1, y.max() + 1]),
...
def main():
    ...
    input_file_path = "Cleaned_IC50.xlsx"
    output_dir = "Scaffold_results_new"
    ...
    # train_idx, val_idx, test_idx = molecular_dataset_split(input_file_path, 
    #                                                       smiles_col= "SMILES", 
    #                                                       method= "scaffold",
    #                                                       frac_train=0.7,
    #                                                       frac_val=0.15,
    ...
    train_df = pd.read_excel("train_new.xlsx")
    val_df = pd.read_excel("val_new.xlsx")
    test_df = pd.read_excel("test_new.xlsx")
    ...
    for epoch in range(1, epochs+1):
        ...
        if val_loss < best_val_loss:
            ...
            counter = 0  # ریست شمارنده Early Stopping
        elif epoch >= 50:  
            counter += 1
            if counter >= patience:
    ...
    # leverages = compute_hat_matrix_approx(test_features,preds_test, trues_test)
    ...
    # leverages = compute_hat_matrix_approx(val_features,preds_val, trues_val)
    ...
    # leverages = compute_hat_matrix_approx(train_features,preds_train, trues_train)
